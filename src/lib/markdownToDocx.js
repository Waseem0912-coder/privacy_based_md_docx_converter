...
// Supports data: URLs (offline) and http/https URLs (may fail due to CORS)
// ─────────────────────────────────────────────────────────────────────────────
...
async function prefetchImages(tree) {
  ...
  await Promise.all([...urls].map(async (url) => {
    try {
      ...
      // CORS, network error, unsupported type — fall back to text placeholder
    }
  }))
...
// ─────────────────────────────────────────────────────────────────────────────
// Build the full Document (title page + TOC + content)
// ─────────────────────────────────────────────────────────────────────────────
function buildDocument(contentElements, docTitle, theme) {
  ...
  const titleElements = [
    ...
    new Paragraph({
      alignment: AlignmentType.CENTER,
      children: [new TextRun({ text: docTitle, bold: true, color: theme.primary, size: 64, font: theme.font })],
      spacing: { before: 0, after: 240 },
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      children: [new TextRun({ text: 'Generated by MD → DOCX Converter', italics: true, color: theme.secondary, size: 24, font: theme.font })],
      spacing: { before: 0, after: 480 },
    }),
    new Paragraph({
      alignment: AlignmentType.CENTER,
      border: { top: { style: BorderStyle.SINGLE, size: 8, color: theme.accent, space: 4 } },
      children: [],
  ...
  if (firstContentIdx >= 0) {
    const first = contentElements[firstContentIdx]
